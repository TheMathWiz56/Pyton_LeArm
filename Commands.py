...
class Command:
    ...
    def initialize_arm(self):
        self.le_arm.go_to(LeArmConstants.GripperState.MIDDLE.value, x=0, z=292.818, pitch=m.pi / 2)

    def go_to(self):
        x = int(input("Enter the X coordinate: "))
        y = int(input("Enter the Y coordinate: "))
        z = int(input("Enter the Z coordinate: "))
        pitch = int(input("Enter the PITCH coordinate (RADIANS): "))
        roll = int(input("Enter the ROLL coordinate (RADIANS): "))
        gripper_state = int(input("Enter 1 for gripper open, 2 fro gripper middle, 3 for gripper closed: "))
        self.le_arm.go_to(get_gripper_state_from_input(gripper_state), x, y, z, pitch, roll)